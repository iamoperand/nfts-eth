...
* rarity (in decreasing order): Pug, Shiba, St. Bernard
...
contract RandomIpfsNft is VRFConsumerBaseV2, ERC721URIStorage, Ownable {
    ...
    constructor(
        ...
        uint256 mintFee
    ) VRFConsumerBaseV2(vrfCoordinatorV2) ERC721("Random Doggie", "RD") {
    ...
    function getBreedFromModdedRng(uint256 moddedRng) public pure returns (Breed) {
        ...
        for (uint256 index = 0; index < chanceArray.length; index += 1) {
            if (moddedRng >= cumulativeSum && moddedRng < cumulativeSum + chanceArray[index]) {
                return Breed(index);
            }
